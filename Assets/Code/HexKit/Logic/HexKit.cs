...
namespace Settworks.Hexagons
{
    public static class HexKit
    {
                                                     ...
                                                     Func<HexRayHit, int?> Attenuation)
        {
            ...
            int ring = 0;
            while (!(arcs.Count() == 1 && arcs[0].intensity == 0))
            {		// Stops when the full circle is dark.
                newArcs.Clear();
                ring++;
                int ringMin = ring * -3;
                int ringMax = ring * 3 - 1;
            ...
            for (int ring = 1; ring <= radius; ring++)
            {
                ...
                foreach (Vector2 arc in arcs)
                {
                    ...
                    if (obstacle)
                    {
                        arcMin = local.PolarBoundingAngle(CCW: true);
                        if (first == ringMin)
                        {		// Even more special handling for prime meridian.
                            float angle = local.PolarBoundingAngle();
                            if (angle < bounds)
                            {
                                bounds = angle;
     ...
     * Pathfinding
        ...
        private static HexPathNode[] GetPath(HexBRPQNode terminus, HexCoord origin, Dictionary<HexCoord, HexBRPQNode> visited)
        {
        ...
        private class RayHit : HexRayHit
        {
