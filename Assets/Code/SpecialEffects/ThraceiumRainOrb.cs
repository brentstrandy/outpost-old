...
public class ThraceiumRainOrb : MonoBehaviour
{
    ...
    //public float ProjectileArch = 0f;
        ...
        if (SessionManager.Instance.GetPlayerInfo().isMasterClient)
        {
            ...
            if (HitTarget)
            {
                ...
                if (other.tag == "Enemy")
                {
                    ...
                    //other.gameObject.GetComponent<Enemy>().TakeDamage(4, 0, );
                }
            }
        }
    }
    ...
    {
        if (ShowDebugLogs)
            Debug.Log("[ThraceiumRain_Orb] " + message);
    }

    private void LogError(string message)
    {
        Debug.LogError("[ThraceiumRain_Orb] " + message);
    }
    #endregion
}
